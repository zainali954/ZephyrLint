...
export default function LoginPage() {
  ...
  const { isLoading, user, accessToken } = useSelector(state => state.auth)
    ...
    <div className="min-h-screen flex items-center justify-center bg-white dark:bg-zinc-900 transition-colors px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Link to={'/forgot-password'} className="text-right text-sm text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer">
            Forgot password?
          </Link>
          <motion.button
            whileTap={{ scale: 0.97 }}
        ...
        <div className="mt-6 text-center text-sm text-zinc-600 dark:text-zinc-400">
          ...
          <Link to={'/signup'} className="text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer">
            Sign up
          </Link>
        </div>
